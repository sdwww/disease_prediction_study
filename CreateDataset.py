...
def create_dataset_jbbm(cursor):
    ...
    dataset_jbbm = np.zeros((patient_num, visit_num, disease_num), dtype='int32')
    print('诊断数据集大小：', dataset_jbbm.shape)
    for i in all_info:
        if i[1] < visit_num:
            dataset_jbbm[i[0], -i[1], i[2]] = 1
    disease_train = dataset_jbbm[:int(patient_num * split_rate)]
    disease_test = dataset_jbbm[int(patient_num * split_rate):]
    np.savez_compressed('./data_npz/disease_train.npz', disease_train=disease_train, disease_test=disease_test)
    # np.savez_compressed('./data_npz/disease_test.npz', disease_test)
    #
    ...
    #     all_info = DBOptions.getSQL(sql, cursor=cursor)
    #     label_jbbm = np.zeros((patient_num, disease_category_num), dtype='int16')
    ...
    #     label_jbbm_train = label_jbbm[:int(patient_num * split_rate)]
    #     label_jbbm_test = label_jbbm[int(patient_num * split_rate):]
    ...
    #
    #     label_sick_train = label_sick[:int(patient_num * split_rate)]
    #     label_sick_test = label_sick[int(patient_num * split_rate):]
...
def create_dataset_drug(cursor):
    ...
    dataset_drug_nocost_train = dataset_drug_nocost[:int(patient_num * split_rate)]
    dataset_drug_nocost_test = dataset_drug_nocost[int(patient_num * split_rate):]
